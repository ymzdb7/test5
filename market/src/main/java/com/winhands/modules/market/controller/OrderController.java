...
/*		Map<String,Object> map1=new HashMap<String,Object>();*/
	...
	public R updateState1(@PathVariable("id")String id){
		...
		if(order.getOrderState().equals("1")||order.getOrderState().equals("2")) {
				order.setOrderState("3");
				orderService.update(order);
					return R.ok();
				}else {
					return R.error("只可对订单状态未付款、已付款进行操作!");
					}
	...
	public R updateState2(@PathVariable("id")String id) {
		...
		//查询订单实体
				...
				if(order.getOrderState().equals("1")||order.getOrderState().equals("2")) {//只有当订单状态为未付款时才可进行订单取消操作，此处作判断
...
/*	
	...
	/**
	 * 客户端获取所有订单接口，用于客户端在手机端操作
	 */
	
	@RequestMapping(value="/orGoodsListApp" , method = RequestMethod.POST)
	public R orGoodsListApp(){
		...
		Map<String, Object> map=new HashMap<String,Object>();
		Map<String, Object> map1=new HashMap<String,Object>();//新增的
		map1.put("address", getUser().getAddress().trim());
		List<OrderEntity> orderList=orderService.queryList(map1);
		if(orderList!=null&&orderList.size()!=0) {
			for (int i = 0; i < orderList.size(); i++) {
				BigDecimal allPrice = new BigDecimal(0);  
		...
		oGmap.put("list", list);
		return R.ok().put("orderMap", oGmap);
		}else {
	...
	public R orderOk(String orderId){
			
				OrderEntity order=orderService.queryObject(Long.valueOf(orderId));
				if(order.getOrderState().equals("1")||order.getOrderState().equals("2")) {
					order.setOrderState("3");
					orderService.update(order);
					return R.ok("已提货！");
